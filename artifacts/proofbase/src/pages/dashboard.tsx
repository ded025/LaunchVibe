      ...
      <div className="min-h-[100dvh] flex flex-col">
        ...
        <main className="flex-1 container mx-auto px-4 py-12 max-w-5xl">
          <div className="animate-pulse space-y-8">
            ...
            <div className="grid md:grid-cols-3 gap-6">{[1, 2, 3].map((i) => <div key={i} className="h-32 bg-muted rounded-xl" />)}</div>
          </div>
        </main>
      </div>
    );
    ...
    <div className="min-h-[100dvh] flex flex-col bg-[#0B0B0C]">
      ...
      <main className="flex-1 container mx-auto px-4 py-12 max-w-5xl">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
          ...
          {/* Gamification bar */}
          {myPoints && (
            <div className="mb-8 p-4 rounded-xl border border-[#1A1A1A] bg-card/50 flex flex-wrap items-center gap-4">
              <div className="flex items-center gap-2">
                <span className="text-2xl">⚡</span>
                <div>
                  <div className="font-bold text-lg text-primary">{myPoints.points} pts</div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="font-bold">{myPoints.streak ?? 0}</div>
              ...
              <div className="ml-auto">
                ...
                >
                  {checkin.isPending ? "Checking in…" : "Daily Check-in +2pts"}
                </Button>
              </div>
            </div>
          )}
          ...
          {(!products || products.length === 0) ? (
            ...
            <>
              ...
              <div className="grid md:grid-cols-4 gap-4 mb-8">
                ...
                <Card className="bg-card/50 border-border">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Feedback</CardTitle>
                  </CardHeader>
              ...
              {feedbackOverTime.length > 0 && (
                <div className="mb-8 p-6 rounded-xl border border-border bg-card/50">
                  <h2 className="text-base font-semibold mb-4 text-muted-foreground uppercase tracking-wide text-xs">Feedback Over Time (30 days)</h2>
                  <ResponsiveContainer width="100%" height={180}>
                    <AreaChart data={feedbackOverTime} margin={{ top: 4, right: 8, bottom: 0, left: -20 }}>
                      <defs>
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex gap-4 text-sm text-muted-foreground flex-wrap">
                              ...
                              {product.wouldPayRatio != null && (
                                <span>{Math.round(product.wouldPayRatio * 100)}% would pay</span>
                              )}
                              {product.score != null && (
